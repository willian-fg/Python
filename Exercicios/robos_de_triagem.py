        ...
        for peso in lista_de_pesos:
                ...
                if (peso >= 100) and (peso <= 300):
                    if resto_da_divisão_do_peso == 0:
                        ...
                        self.total_de_pecas_aprovadas = self.total_de_pecas_aprovadas + 1
                        self.peso_total_processado = self.peso_total_processado + peso

                elif (peso == 99) or (peso == 399):
                    print("Alerta, Peso crítico detectado. Travando esteira!")
                    break
                
                else:
                    print("Pesso {peso} reprovado.")
                    self.total_de_pecas_reprovadas = self.total_de_pecas_reprovadas + 1
                    self.peso_total_processado = self.peso_total_processado + peso
        

    def relatorio(self):
        print(f"""
        Total de Peças Aprovadas: {self.total_de_pecas_reprovadas}\n
